...
logger = logging.getLogger("DocBuilder.NRBFParser")
...
class NRBFParser:
    ...
    def parse(self):
        try:
            ...
            logger.error(f"NRBF parsing error: {e}", exc_info=True)
        return self.objects
    ...
    def _parse_records(self):
        while True:
            ...
            logger.debug(f"Pos {pos:05d}: Record Type {rec_type} ({rec_name})")
    ...
    def _parse_record(self, rec_type):
        if rec_type == SerializedStreamHeader:
        ...
        elif rec_type in (ClassWithMembersAndTypes, SystemClassWithMembersAndTypes):
            pos_id = self.f.tell()
            obj_id = struct.unpack("<i", self.f.read(4))[0]
            ...
            logger.debug(f"  [DEBUG Class] pos_id={pos_id}, obj_id={obj_id}, pos_name={pos_name}, name='{name}', pos_mc={pos_mc}, member_count={member_count}")
            ...
            for bt in binary_types:
                ...
                elif bt in (3, 4): # SystemClass or Class
            ...
            logger.debug(f"  Class: {name}, member_names: {member_names}, binary_types: {binary_types}")
